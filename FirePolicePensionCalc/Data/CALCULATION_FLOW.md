...
**Result:** `disbursementResult.totalPayout` = total lifetime benefit in **today's buying power** (inflation-adjusted)
...
**Note on Units:** This value is in "today's buying power" because inflation is applied each year to show the real purchasing power of the payments. This is useful for understanding the benefit value from the retiree's perspective.
...
- Annual pension: $54,375
- Years retired: 23
- **Amount needed at retirement: $1,250,625** (full sum of all payments in **nominal dollars**)

**⚠️ IMPORTANT: Units Distinction**

The system uses **two different units** for different purposes:

1. **`totalPayout`** (Step 1): In **today's buying power** (inflation-adjusted)
   - Purpose: Shows the real value of benefits from the retiree's perspective
   - Calculation: Sum of payments with COLA increases, but adjusted for inflation each year
   - Use: Display and understanding of benefit value

2. **`amountNeededAtRetirement`** (Step 2): In **nominal dollars** (retirement-date dollars)
   - Purpose: Determines actual dollars needed for funding
   - Calculation: Sum of payments with COLA increases, but **without** inflation adjustments
   - Use: Funding calculations - this is the actual dollar amount that must be available

**Why Different Units?**

- **For funding**, we need **nominal dollars** because that's what will actually be paid out. The pension fund must have the actual dollar amounts, not inflation-adjusted values.
- **For display**, we use **buying power** to help people understand the real value of their benefits in today's terms.

**Example to Illustrate:**
- Year 1 payment: $54,375 (nominal)
- After 10 years with 2.63% inflation: $54,375 still paid (nominal), but worth ~$41,200 in today's buying power
- For funding: We need $54,375 × 23 = $1,250,625 (nominal dollars)
- For display: We show ~$922,560 (today's buying power) to help understand real value

This distinction is intentional and correct for actuarial funding purposes.
